...
(function ($) {
	...
	function launch(elem) {
		...
		if (rel && rel !== 'nofollow') {
			$related = jQuery('.cboxElement').filter(function () {
	...
	cboxPublic = jQuery.fn.colorbox = function (options, callback) {
		var $this = this;
		...
		$this.each(function () {
			var data = jQuery.extend({}, jQuery(this).data(colorbox) ? jQuery(this).data(colorbox) : defaults, options);
			
			jQuery(this).data(colorbox, data).addClass("cboxElement");
			
			if (callback) {
				jQuery(this).data(colorbox).onComplete = callback;
			}
		});
	...
	cboxPublic.init = function () {
		...
		$loadingBay = jQuery("<div style='position:absolute; top:0; left:0; width:9999px; height:0;'/>");
		
		jQuery('body').prepend($overlay, $cbox.append($wrap, $loadingBay));
		...
		jQuery('.cboxElement').live('click', function (e) {
	...
	cboxPublic.resize = function (object) {
		...
		$loaded.remove();
		$loaded = jQuery('<div id="cboxLoadedContent"/>').html(object);
		...
		jQuery('#cboxPhoto').css({cssFloat:'none'});// floating the IMG removes the bottom line-height and fixed a problem where IE miscalculates the width of the parent element as 100% of the document width.
	...
	cboxPublic.load = function () {
		...
		*/
		...
		href = settings.href || jQuery(element).attr("href");
		...
		if (settings.inline) {
			...
			img.onload = function(){
				...
				if($related.length > 1){
					jQuery(img).css({cursor:'pointer'}).click(cboxPublic.next);
				}
			...
			jQuery('<div />').appendTo($loadingBay).load(href, function(data, textStatus){
	...
	cboxPublic.close = function () {
		...
		open = FALSE;
		jQuery().unbind("keydown.cbox_close keydown.cbox_arrows");
	...
	cboxPublic.element = function(){ return jQuery(element); };
